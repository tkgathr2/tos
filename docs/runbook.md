...
**本 Runbook は最終版です。変更は禁止されています。**

TOS v0.3 S-6 stable は 2025-12-14 に完結・封印されました。

- **変更禁止**: 本ドキュメントへの変更は原則禁止
- **例外なし**: 変更を許可する例外条件は存在しない
- **運用のみ**: 本 Runbook は運用目的でのみ使用可能
- **改修依頼不可**: 機能追加・仕様変更の依頼は受け付けない

封印宣言: [S-6 最終封印宣言](s6_sealed.md) | 最終完了宣言: [TOS v0.3 FINAL](tos_v0_3_final.md)
...
| 週次 | 毎週 | logs/ 肥大化確認、状態確認 |
| 月次 | 毎月 | ドキュメント整合性、ログ整理 |
| 障害対応 | 随時 | 一次対応 → 原因調査 → rollback → 報告 |
...
- [S-6 Stable](s6_stable.md) - フェーズ定義・運用ルール
- [S-6 Operations](s6_operations.md) - 安定運用ガイド（運用フロー・ログ・組織）
- [S-6 Stable Checklist](s6_stable_checklist.md) - 運用チェックリスト

---

## S-7 運用ステータス（不要・確定）

**S-7 は運用対象外です。S-7 は 2025-12-14 に「不要」と確定しました。**

### S-7 に関するルール

1. **S-7 の実装は行わない**
2. **S-7 の再検討は 2025-06-14 以降のみ許可**
3. **再検討にはトリガー条件の発生が必要**（ディスク容量80%、APIエラー週5回以上等）
4. **「あると便利」レベルの機能追加要求は却下**
...
TOS v0.3 S-6 stable の運用に必要なすべての情報が本ドキュメントに記載されています。

- 追加の手順書は作成されません
- 新しいモードや機能は追加されません
- 本ドキュメントが最終版です

運用上の不明点は [s6_routines.md](s6_routines.md) または [s6_one_pager.md](s6_one_pager.md) を参照してください。

封印宣言: [S-6 最終封印宣言](s6_sealed.md)
