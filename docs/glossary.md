...
| S-7 | **不要（確定）** | 2025-12-14 に「やらない」と確定。再検討は 2025-06-14 以降 |
...
完成したシステム・フェーズに対して、これ以上の成長・拡張を行わないことを宣言すること。

- **対象**: S-6 stable（2025-12-14 封印）
- **許可操作**: 運用のみ（日次/週次/月次ルーチン、障害対応）
- **禁止操作**: 機能追加、仕様変更、設定スキーマ変更、新モード追加
- **解除条件**: 原則として存在しない

封印されたシステムは「育てる対象」ではなく「運用する対象」である。

### FINAL（最終完了）

TOS v0.3 が完成・封印・運用のみの状態になったことを示す宣言。

- **宣言日**: 2025-12-14
- **意味**: 開発フェーズ終了、運用フェーズへ完全移行
- **許可操作**: 運用のみ
- **禁止操作**: 実装変更、仕様変更、拡張検討

詳細は [TOS v0.3 FINAL](tos_v0_3_final.md) を参照。
